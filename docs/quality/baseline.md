...
# TASK-10 quality baseline

## TASK-10-RECONCILE-01 verification (2026-08-08)

The Java 21 gate was executed from a clean Maven `target` state with
`JAVA_HOME=D:\Env\JDK\MicrosoftOpenJDK21`:

```powershell
./mvnw -B clean verify
```

Result: `BUILD SUCCESS` in 3 minutes 50 seconds. Surefire reported 209 tests,
0 failures, 0 errors, and 0 skipped: common 5, domain 9, database 32,
security 30, task 60, portal 52, and admin 21. The run started fresh MySQL
8.0.46, RabbitMQ 4.0.7, and Redis 8.8.1 Testcontainers; no infrastructure test
was disabled or skipped.

Reconciliation-specific evidence in that run includes:

- `PaymentTerminalRaceContainerTest` invokes production
  `MockPaymentCallbackService.accept` with an HMAC generated by the production
  `MockPaymentProvider`, while the competing path invokes transactional
  `OrderTimeoutService.process`. It covers payment-first, timeout-first with a
  real late callback, duplicate callbacks with a new nonce, and 20 latch-based
  concurrent rounds without fixed sleeps or copied state-transition SQL.
- `ReliableMessagingContainerTest` has 23 tests, including ordinary and
  SECKILL timeout transaction rollback/retry, append-only compensation audit,
  a persistent HTTP 503 callback driven exactly to the configured attempt
  limit, deterministic 400 DLQ routing, and a real broker publish NACK proving
  that the original delivery is not acknowledged early.
- `SeckillPaymentExpiredDeliveryContainerTest` has 6 real RabbitMQ + Redis
  tests covering normal and duplicate compensation, strict poison/fact-conflict
  DLQ routing, transient Redis recovery through a durable TTL retry queue,
  retry exhaustion, and retry-publish confirm failure.

The remaining gates also passed: `docker compose config --quiet`; `pnpm check`
(Prettier, ESLint, TypeScript, 14 Vitest tests, production build, and generated
client drift); and `pnpm test:e2e` with 6/6 Chromium desktop/mobile tests.

Mock Payment 门禁包含 MySQL 8 空库/V1.5 升级、约束/无外键、并发唯一创建、HMAC 原始字节覆盖、timestamp/nonce/callbackId 冲突、金额事实、重复效果、失败/成功/迟到状态、审计回滚、20 轮真实数据库终态竞态、普通与秒杀库存一次性补偿、真实 Redis 投影幂等、真实 RabbitMQ durable routing/retry/DLQ，以及 User/Admin/Agent/anonymous/OpenAPI 边界。并发测试只使用 latch 和数据库锁，不使用固定等待猜测顺序。
